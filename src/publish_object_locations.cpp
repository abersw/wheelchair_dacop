...
void objectLocationsCallback(const wheelchair_msgs::objectLocations obLoc) {
    ...
    for (int isMsgObject = 0; isMsgObject < totalObjectsInMsg; isMsgObject++) { //run through detected objects
    ...
    //temp print for existing objects
    for (int isExisObj = 0; isExisObj < totalExistingObjects; isExisObj++) {
        cout << "is object " << isExisObj << 
        "total existing items " << totalExistingObjects << endl <<
        "id " << existingObjects[isExistingObject].id << endl <<
        "name " << existingObjects[isExistingObject].object_name << endl <<
        "confidence " << existingObjects[isExistingObject].object_confidence << endl <<

        "x " << existingObjects[isExistingObject].point_x << endl <<
        "y " << existingObjects[isExistingObject].point_y << endl <<
        "z " << existingObjects[isExistingObject].point_z << endl <<

        "x " << existingObjects[isExistingObject].quat_x << endl <<
        "y " << existingObjects[isExistingObject].quat_y << endl <<
        "z " << existingObjects[isExistingObject].quat_z << endl <<
        "w " << existingObjects[isExistingObject].quat_w << endl;
    }
